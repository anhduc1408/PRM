...
// ─────────────────────────────────────────────────────────────────────────────
// Màn hình MANAGER - Xác nhận nhận hàng từ phiếu chuyển kho
// ─────────────────────────────────────────────────────────────────────────────
class ManagerReceiveScreen extends StatefulWidget {
  const ManagerReceiveScreen({super.key});

  @override
  State<ManagerReceiveScreen> createState() => _ManagerReceiveScreenState();
}

class _ManagerReceiveScreenState extends State<ManagerReceiveScreen>
  ...
  Future<void> _loadData() async {
    ...
    final storeId = authProv.currentUser?.storeId;

    if (storeId != null) {
      // Find the specific warehouse representing this store
      final whProv = context.read<WarehouseProvider>();
      // Try to load warehouses if not loaded yet
      if (whProv.warehouses.isEmpty) {
        await whProv.loadAll();
      }
      
      final storeWarehouses = whProv.warehousesForStore(storeId);
      // Lấy kho có type là 'store' (trường hợp store_id=1 có bị dính kho tổng thì bỏ ra)
      final myWarehouse = storeWarehouses.where((w) => w.type == 'store').firstOrNull ?? storeWarehouses.firstOrNull;

      if (myWarehouse != null) {
        _myWarehouseId = myWarehouse.id;
        final transfers = await whProv.loadIncomingTransfers(_myWarehouseId!);
        if (mounted) {
          setState(() {
      ...
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          if (isActionable) ...[
            ...
            const Text('Kho hàng đang ổn định!',
        ...
        child: Column(children: [
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${t.fromWarehouseName ?? '?'} → Cửa hàng',
                    style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
              ])),
              IconButton(
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Icon(Icons.info_outline, size: 14, color: Color(0xFF1A237E)),
                  const SizedBox(width: 6),
                  const Expanded(
                    child: Text('Nhập số lượng thực tế nhận được cho từng sản phẩm.',
                        style: TextStyle(fontSize: 12, color: Color(0xFF1A237E), fontWeight: FontWeight.w500)),
                  ),
                ]),
                const SizedBox(height: 4),
                Text(
            ...
            child: Row(children: const [
              ...
              SizedBox(
                ...
                child: Text('Thực nhận', textAlign: TextAlign.center,
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Color(0xFF1A237E))),
              ),
            ]),
          ),
          const SizedBox(height: 4),
